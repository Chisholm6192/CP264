...
void test_select_sort_inc() {
	printf("------------------\n");
	printf("Test: select_sort_inc\n\n");
	float *a[MAX_LEN];
	...
	for (int i = 0; i < count; i++) {
		...
		printf("select_sort_inc(");
		display_array(a, left, right);
		printf("): ");
		select_sort_inc(a, left, right);
		display_array(a, left, right);
		printf("\n");
	}
	printf("\n");
...
void test_quick_sort_inc() {
	printf("------------------\n");
	printf("Test: quick_sort_inc\n\n");
	float *a[MAX_LEN];
	int count = sizeof tests / sizeof *tests;
	for (int i = 0; i < count; i++) {
		int left = tests[i][0];
